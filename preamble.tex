...
% tabularray registers a hook to load ninecolors after xcolor.
% ninecolors uses rgb:Hsb which requires xcolor 3.x (not available on Ubuntu's TeX Live).
% We don't use ninecolors colors, so remove the hook before xcolor loads.
\RemoveFromHook{package/xcolor/after}[tabularray]
