...
Chapter~9 to work building real monitoring scripts.
...
\begin{challengebox}
Use the loop technique from Chapter~9 to record \texttt{si} and \texttt{so} values
...
\textit{Observe:} The \texttt{/swapfile-week10} entry should appear in
...
\subsection*{Lab 10.4 Memory Monitoring with a Script}
...
\textbf{Step 2}: Build an automated monitoring script applying Chapter~9 concepts
...
\texttt{memory-YYYYMMDD-HHMMSS.txt} using \texttt{date} (a technique from Chapter~9)
...
\subsection*{Lab 10.5 Observing System Calls with strace}
