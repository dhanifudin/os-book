%% ========================================================================
%% Chapter 8: Memory Management and System Call
%% ========================================================================
\chapter{Memory Management and System Call}
\label{ch:memory-and-syscalls}

\chapterhero{blue}{Connect RAM, swap, and memory pressure concepts with how programs talk to the kernel through system calls and Linux diagnostic tools.}{\faMemory\quad memory}{\faHdd\quad swap}{\faSearch\quad strace}
...
\begin{itemize}
    \item \textbf{Virtual memory architecture}: each process has its own virtual address space, which the kernel maps to physical RAM through the \term{page table}.
    \item \textbf{Paging activity}: \cmd{vmstat} shows the \texttt{si}/\texttt{so} columns for swap-in and swap-out; high values indicate memory pressure.
    \item \textbf{Swap configuration}: a swap file is created with \cmd{fallocate}, formatted with \cmd{mkswap}, and enabled with \cmd{swapon}; \texttt{vm.swappiness} controls how aggressively swap is used.
    \item \textbf{Memory monitoring}: \cmd{free -h}, \cmd{top}/\cmd{htop}, and \cmd{/proc/meminfo} report RAM and swap usage from different viewpoints; Bash scripts can automate periodic checks.
    \item \textbf{Monitoring scripts}: Bash techniques from Chapter~7, including functions, loops, and \cmd{tee}, can be combined to build \texttt{monitor-memory.sh} with automatic timestamped logs.
    \item \textbf{System calls}: programs interact with the kernel through system calls; \cmd{strace} displays those calls so you can analyze program behavior and errors.
\end{itemize}
