...
\begin{table}[htbp]
	...
	\begin{tabular}{@{}lp{10cm}@{}}
		...
		\verb|>>|         & Redirect stdout to file (append)             \\
		\texttt{2>}       & Redirect stderr to file (overwrite)          \\
		\verb|2>>|        & Redirect stderr to file (append)             \\
		\texttt{2>\&1}    & Redirect stderr to stdout                    \\
		\texttt{\&>}      & Redirect stdout and stderr to file (Bash 4+) \\
		\verb|<<|         & Here document (multi-line input)             \\
		\verb|<<<|        & Here string (single string input)            \\
		\bottomrule
	\end{tabular}
\end{table}
...
\begin{exercisebox}[3.1]
	Run the following commands one at a time in the terminal to display and save the 10 largest files in \dir{/var/log}:
	\begin{enumerate}
		\item Display the 10 largest files in \dir{/var/log}: \texttt{du -ah /var/log 2>/dev/null | sort -rh | head -10}
		\item Save the results to \file{large-logs.txt}: \texttt{du -ah /var/log 2>/dev/null | sort -rh | head -10 > large-logs.txt}
		\item Run the same command using \cmd{tee} so output appears on terminal and is saved to file simultaneously
		\item Separate error messages to \file{error.log} using \texttt{2>}
	\end{enumerate}
\end{exercisebox}
...
\begin{exercisebox}[3.3]
	Use redirection to record system information to a log file. Run each command one at a time in the terminal:
	\begin{enumerate}
		\item Create a new log file with a timestamp header: \texttt{date > system-monitor.log}
		\item Append memory information to the log: \texttt{free -h >> system-monitor.log}
		\item Append disk information to the log: \texttt{df -h >> system-monitor.log}
		\item Display running processes and save simultaneously: \texttt{ps aux | tee -a system-monitor.log | head -5}
		\item Display the contents of the log file: \texttt{cat system-monitor.log}
	\end{enumerate}

	Notice the difference between \texttt{>} (overwrite) and \texttt{>>} (append) at each step.
\end{exercisebox}
...
\begin{exercisebox}[3.5]
	Back up a practice directory using \cmd{tar} with redirection to separate normal output from error messages. Run each command in the terminal:
	\begin{enumerate}
		\item Create a practice directory first: \texttt{mkdir -p \textasciitilde/practice}
		\item Create a backup and separate output to two different files:\\
		      \texttt{tar -czvf backup-practice.tar.gz \textasciitilde/practice > backup-output.log 2> backup-error.log}
		\item View the output file contents: \texttt{cat backup-output.log}
		\item View the error file contents (if any): \texttt{cat backup-error.log}
		\item List archive contents and save the list simultaneously: \texttt{tar -tzvf backup-practice.tar.gz | tee backup-contents.log}
	\end{enumerate}

	Notice how stdout (list of backed-up files) and stderr (error messages) are separated into different files.
\end{exercisebox}
