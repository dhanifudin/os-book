...
cd ~/lab-os
tar -czvf chapter04-backup.tar.gz chapter04
ls -lah chapter04-backup.tar.gz
\end{lstlisting}

    \item List the archive contents without extracting:
\begin{lstlisting}[language=bash, caption={Listing tar.gz contents}]
tar -tzvf chapter04-backup.tar.gz | head -n 30
\end{lstlisting}
...
tar -xzvf chapter04-backup.tar.gz \
  -C ~/lab-os/restore-test
ls -lah ~/lab-os/restore-test/chapter04 | head
\end{lstlisting}
\end{enumerate}
...
Create the folder \texttt{\~/lab-os/chapter04-exercises} containing:
\begin{itemize}
