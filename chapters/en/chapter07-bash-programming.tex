...
\chapterhero{purple}{Turn manual command sequences into clean, reusable, and safer scripts through Bash functions, validation, and debugging.}{\faFileCode\quad script}{\faPuzzlePiece\quad function}{\faBug\quad debugging}
...
\begin{enumerate}
    \item create executable Bash scripts with the correct shebang and permissions, and understand the role of exit codes;
    \item use variables, positional parameters \texttt{\$1}, \texttt{\$\#}, \texttt{\$@}, and Bash arithmetic;
    \item write \cmd{if}/\cmd{elif}/\cmd{else}, \cmd{for}, \cmd{while}, and \cmd{case} control structures to automate decision logic;
    \item define shell functions with local variables and separate functions into library files that can be \cmd{source}d;
    \item debug scripts using \cmd{set -x}, \cmd{set -e}, \cmd{set -u}, and \cmd{trap} for automatic cleanup.
\end{enumerate}
...
\subsection*{Lab 6.1 First Script: System Report}
...
\subsection*{Lab 6.2 System Info Script with Arguments}
...
\subsection*{Lab 6.3 Grading Script and Interactive Menu}
...
\subsection*{Lab 6.4 Validation Function Library}
...
\subsection*{Lab 6.5 Backup Script with Options}
...
\subsection*{Lab 6.6 Debugging a Script}
