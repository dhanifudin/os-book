...
ls -lah /etc/ssh/
sudo test -f /etc/ssh/sshd_config && echo "sshd_config ada" || echo "sshd_config tidak ditemukan"
\end{lstlisting}
...
sudo grep -E "Failed password|Accepted password|Invalid user" /var/log/auth.log | tail -n 30
\end{lstlisting}
\end{enumerate}
...
\end{lstlisting}
    ...
    \item Gunakan \texttt{pushd/popd} untuk bolak-balik:
...
\subsection*{chmod: notasi angka (oktal) + tabel konversi}
Dalam notasi oktal, setiap triad (\textit{user/group/others}) direpresentasikan oleh angka 0--7:
\begin{center}
\begin{tabular}{lll}
...
Contoh:
\begin{itemize}
    \item \texttt{chmod 755 file} $\rightarrow$ owner \texttt{rwx}, group \texttt{r-x}, others \texttt{r-x}
    \item \texttt{chmod 640 file} $\rightarrow$ owner \texttt{rw-}, group \texttt{r--}, others \texttt{---}
\end{itemize}
...
ls -ld permtest/dirA
cd permtest/dirA 2>/dev/null || echo "Gagal masuk karena tidak ada execute (x) pada direktori"
\end{lstlisting}
\end{enumerate}
...
touch /srv/project-gamma/test-$USER.txt 2>/dev/null || echo "Jika gagal: re-login atau jalankan newgrp project"
...
cat hardlink.txt
cat symlink.txt || echo "symlink broken karena target hilang"
\end{lstlisting}
\end{enumerate}
...
sudo find / -type f -name "*.conf" 2>/dev/null | head -n 30
\end{lstlisting}
...
sudo find / -type f -perm -4000 2>/dev/null | head -n 30   # setuid
sudo find / -type f -perm -2000 2>/dev/null | head -n 30   # setgid
\end{lstlisting}
\end{enumerate}
...
tar -xzvf week04-backup.tar.gz -C ~/praktikum-os/restore-test
...
tar -tzvf ~/backup/etc-core-$(date +%F).tar.gz | head -n 40
\end{lstlisting}
...
tar -xzvf ~/backup/etc-core-$(date +%F).tar.gz -C ~/backup/restore-sandbox
