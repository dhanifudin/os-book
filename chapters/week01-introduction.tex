...
Perdebatan famous antara Andrew Tanenbaum (creator of MINIX, microkernel) dan Linus Torvalds (creator of Linux, monolithic):

\textbf{Tanenbaum}: "Linux is obsolete" - argued monolithic kernels are outdated

\textbf{Torvalds}: Defended pragmatic monolithic design - performance dan simplicity

\textbf{Outcome}: Both approaches have merits. Linux dominates servers dengan monolithic-modular design, sementara QNX microkernel dominates automotive systems.
\end{examplebox}

\subsection{System Calls}

\textbf{System call} adalah interface antara user space program dan kernel services. Ini adalah controlled entry point ke kernel.
...
Proses ini sama conceptually di semua OS, tapi implementation details berbeda.
\end{examplebox}
...
\begin{description}
    ...
    \item[1960s: Time-Sharing] CTSS (Compatible Time-Sharing System) di MIT - multiple users sharing computer time. Multics project (ambitious tapi gagal, inspires Unix).

    \item[1969: Lahirnya Unix] Ken Thompson dan Dennis Ritchie di Bell Labs menciptakan Unix. Simple, portable (ditulis dalam C), filosofi "do one thing well".

    \item[1980s: Era PC] MS-DOS (1981) untuk IBM PC. Apple Macintosh (1984) dengan GUI. Windows 1.0 (1985) sebagai shell di atas DOS.

    \item[1991: Linux] Linus Torvalds, mahasiswa dari Finland, merilis Linux kernel 0.01. Free, open source, Unix-compatible.

    \item[1993: Windows NT] Microsoft merilis Windows NT dengan hybrid kernel architecture untuk enterprise market.

    \item[2001: Mac OS X] Apple merilis macOS X berbasis Darwin (BSD Unix + Mach microkernel). UNIX certified.

    \item[2000s: Modern Era] Windows XP unifies consumer dan NT lines. Linux dominates server market. Open source momentum.

    \item[2007--2008: Mobile Revolution] iOS (2007) dan Android (2008) mendefinisikan ulang computing dengan mobile-first approach.

    \item[2010s: Cloud Era] Linux dominates cloud computing (AWS, Google Cloud, Azure). Containers (Docker) revolutionize deployment.

    \item[2020s: Current] Hybrid computing, ARM processors (Apple Silicon), WSL brings Linux to Windows, security-focused development.
\end{description}
...
Linux bukan single OS, tapi collection of distributions yang share Linux kernel tapi berbeda dalam package management, default software, dan target audience.
...
\textbf{Keuntungan:}
\begin{itemize}
    \item Safe experimentation - tidak affect host system
    \item Snapshots untuk easy recovery
    \item Multiple OS secara parallel
...
\textbf{Risk:} Partition errors, bootloader conflicts, data loss jika tidak careful
...
\begin{itemize}
    ...
    \item Network: NAT atau Bridged (untuk network access)
\end{itemize}
...
\begin{enumerate}
    ...
    \begin{itemize}
        \item DHCP (automatic) - recommended untuk beginning
        \item Atau configure static IP (advanced)
    \end{itemize}
    \item Proxy: Leave blank (unless needed)
...
\textbf{Option 1: Guided - use entire disk (Recommended untuk beginners)}
\begin{itemize}
    \item Automatic partitioning
    \item Sets up /, /boot/efi, dan swap
    \item Simple, fast
\end{itemize}
...
\begin{itemize}
    \item Installation akan download dan install packages (~15-20 minutes)
    \item Wait hingga "Installation complete!" muncul
    \item Select "Reboot Now"
    \item Remove installation media (VirtualBox auto-ejects)
\end{itemize}
...
Setelah instalasi successful, create snapshot di VirtualBox:
\begin{enumerate}
    \item Power off VM
...
Ini memungkinkan restore ke state bersih kapanpun dibutuhkan.
\end{examplebox}
...
\begin{itemize}
    ...
    \item \textbf{Multi-OS} - Install multiple OS pada satu disk
    \item \textbf{Backup/Recovery} - Easier to backup specific partitions
    \item \textbf{Security} - Isolate data dengan different permissions
    \item \textbf{Performance} - Optimize different workload types
\end{itemize}
...
\begin{itemize}
    ...
    \item Extended partition dapat contain multiple logical partitions
...
\begin{itemize}
    ...
    \item Required untuk UEFI Secure Boot
...
Benefits:
\begin{itemize}
    \item Reinstall OS tanpa lose user data
    \item Easier backup (backup only \texttt{/home})
    \item Upgrade distro dengan preserve settings
    \item Different filesystem atau encryption untuk data
\end{itemize}
\end{tipbox}
...
\begin{itemize}
    \item Saat RAM penuh, inactive pages moved to swap
    \item Hibernation (suspend-to-disk) - save RAM contents to swap
\end{itemize}
...
\begin{itemize}
    \item Default untuk most Linux distributions
    \item Mature, stable, well-tested
    \item Journaling untuk crash recovery
    \item Good performance untuk general use
\end{itemize}
...
\begin{itemize}
    ...
    \item Excellent untuk large files dan parallel I/O
    \item Default di Red Hat Enterprise Linux
    \item Cannot shrink (only grow)
\end{itemize}
...
\begin{itemize}
    ...
    \item Built-in snapshots dan compression
    \item RAID support
    \item Still maturing (production-ready di openSUSE)
\end{itemize}

\subsection{Praktik: Manual Partitioning}

Untuk advanced users, manual partitioning allows full control.

Durante Ubuntu installation, select "Custom storage layout":
...
Use UUID instead of \texttt{/dev/sdX} di fstab karena:
\begin{itemize}
    \item Device names can change (sda → sdb jika add disk)
    \item UUID adalah persistent identifier
    \item More reliable untuk production systems
\end{itemize}

Get UUID: \cmd{blkid} atau \cmd{lsblk -f}
\end{notebox}
...
Setelah instalasi completed, several konfigurasi penting harus dilakukan.
...
# Remove packages yang tidak dibutuhkan
sudo apt autoremove
...
\begin{enumerate}
    \item OS concepts are universal - applicable across platforms
    \item Linux excellent untuk learning: open, free, industry-relevant
    \item Virtual machines provide safe learning environment
    \item Proper partitioning important untuk flexibility dan recovery
    \item System updates should be performed regularly
    \item Basic system monitoring tools membantu understanding system state
\end{enumerate}
...
\begin{enumerate}
    \item Download Ubuntu Server ISO dari website resmi
    \item Create VM baru di VirtualBox (RAM: 2GB, Disk: 25GB)
    ...
    \item Dokumentasikan proses instalasi dengan screenshot key steps
\end{enumerate}
\end{exercisebox}
