...
\begin{enumerate}
    ...
    \item menginstalasi Ubuntu Server 24.04 LTS di mesin virtual VirtualBox;
    \item menjelaskan konsep partisi disk dan tipe sistem berkas di Linux;
...
\begin{description}[noitemsep,topsep=2pt,parsep=0pt]
    ...
    lapisan abstraksi melalui driver sehingga program tidak perlu memahami cara kerja
...
\begin{notebox}
Tanpa sistem operasi, setiap program harus dilengkapi driver sendiri untuk setiap
...
\subsection{Ruang Kernel dan Ruang Pengguna}

Sistem operasi modern membagi memori menjadi dua wilayah yang terpisah secara ketat:

\begin{description}[noitemsep,topsep=2pt,parsep=0pt]
    \item[Ruang Kernel (Ring 0)] Wilayah dengan hak akses tertinggi. Hanya kode kernel
    yang berjalan di sini. Kode di ruang kernel dapat membaca dan menulis ke semua
    alamat memori, serta berbicara langsung ke perangkat keras.

    \item[Ruang Pengguna (Ring 3)] Wilayah tempat aplikasi biasa berjalan. Akses ke
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[node distance=1.2cm, scale=0.95]
        \node[draw, rectangle, minimum width=11cm, minimum height=1.4cm,
            ...
            \textbf{Ruang Pengguna (Ring 3)}\\[0.2em]
        ...
        \node[draw, rectangle, minimum width=11cm, minimum height=1.8cm,
            ...
            \textbf{Ruang Kernel (Ring 0)}\\[0.2em]
...
\texttt{open()}. Kernel kemudian memvalidasi izin, berinteraksi dengan driver disk, dan
mengembalikan hasilnya kepada program yang memanggil.

\subsection{Jenis Arsitektur Kernel}

Ada tiga pendekatan utama dalam merancang kernel sistem operasi:
...
\begin{table}[htbp]
    ...
    \begin{tblr}{
        ...
        Deskripsi &
            Semua layanan berjalan di ruang kernel dalam satu binary besar &
            Hanya layanan esensial di kernel; sisanya berjalan di ruang pengguna &
...
driver dapat ditambahkan tanpa harus mengompilasi ulang seluruh kernel.
...
\begin{figure}[H]
    ...
    \caption{Perbandingan arsitektur kernel: area merah menunjukkan layanan di ruang
    kernel, area biru di ruang pengguna}
...
sama, tetapi berbeda dalam pengelola paket perangkat lunak, konfigurasi bawaan, dan
target penggunanya.
\end{notebox}
...
Praktikum pada bab ini menggunakan \textbf{mesin virtual} (virtual machine). Mesin
virtual adalah komputer yang disimulasikan di dalam komputer fisik menggunakan perangkat
lunak. Pendekatan ini aman untuk belajar: jika terjadi kesalahan konfigurasi, mesin
virtual dapat dikembalikan ke kondisi semula tanpa memengaruhi sistem utama sama sekali.

\begin{notebox}
Seluruh praktikum pada bab ini dijalankan dari antarmuka teks (command line) Ubuntu
...
\subsection*{Praktikum 1.1 Membuat Mesin Virtual}

Buat sebuah mesin virtual baru di VirtualBox untuk menginstalasi Ubuntu Server.
...
\begin{enumerate}
    ...
    alokasi dinamis.
    \item Setelah mesin virtual terbentuk, buka \textbf{Settings} lalu navigasi ke
    \textbf{Storage}.
    \item Pada Controller: IDE, tambahkan optical drive dan pilih berkas ISO Ubuntu
    Server yang sudah diunduh.
    \item Pastikan tab \textbf{Network} menggunakan mode \textbf{NAT} agar mesin virtual
    dapat mengakses internet.
    \item Klik \textbf{Start} untuk menyalakan mesin virtual.
...
\subsection*{Praktikum 1.2 Proses Instalasi Ubuntu Server}
...
\subsection*{Praktikum 1.3 Login Pertama dan Verifikasi Sistem}
...
\begin{tipbox}
Setelah instalasi berhasil, segera buat snapshot di VirtualBox. Matikan mesin virtual,
...
dilakukan sebelum sistem siap digunakan untuk praktikum selanjutnya.

\subsection*{Praktikum 1.4 Memperbarui Sistem}
...
# Unduh informasi paket terbaru dari repositori
sudo apt update
...
hanya memperbarui daftar paket yang tersedia dari server repositori. Perintah
\cmd{apt upgrade} baru melakukan instalasi berdasarkan daftar yang sudah diperbarui.

\subsection*{Praktikum 1.5 Instalasi Utilitas Dasar}

Instalasi beberapa utilitas yang akan sering digunakan sepanjang praktikum di buku ini:
...
\subsection*{Praktikum 1.6 Verifikasi Koneksi dan Sumber Daya Sistem}
...
\section{Partisi Disk dan Sistem Berkas}
...
partisi bersifat independen, dapat menggunakan sistem berkas yang berbeda, dan dapat
diproteksi secara terpisah.

\subsection{Tabel Partisi: MBR dan GPT}

Ada dua standar tabel partisi yang digunakan saat ini:
...
\subsection{Tata Letak Partisi Linux}

Saat memilih \textit{Use an entire disk} pada Praktikum 1.2, Ubuntu membuat tata letak
partisi berikut secara otomatis:
...
\subsection{Tipe Sistem Berkas}

Setiap partisi diformat dengan sistem berkas tertentu yang menentukan bagaimana data
disimpan dan diorganisasikan:

\begin{table}[htbp]
    \centering
    \caption{Tipe sistem berkas umum di Linux}
    ...
    \begin{tblr}{
        ...
        Sistem Berkas & Karakteristik & Digunakan untuk \\
        ext4 & Stabil, matang, mendukung journaling & Partisi root (\texttt{/}) dan data \\
        swap & Area memori virtual di disk & Partisi swap \\
        FAT32 / vfat & Kompatibel dengan semua firmware & Partisi \texttt{/boot/efi} \\
    \end{tblr}
\end{table}

\begin{notebox}
Tipe sistem berkas yang lebih canggih seperti XFS dan Btrfs dibahas pada Minggu 5
bersama dengan topik manajemen berkas tingkat lanjut.
\end{notebox}

\subsection*{Praktikum 1.7 Memeriksa Partisi dan Sistem Berkas}

Setelah sistem terinstalasi, periksa tata letak partisi yang dibuat secara otomatis.

\begin{lstlisting}[language=bash, caption={Memeriksa partisi dan sistem berkas}]
# Tampilkan tata letak disk dan partisi dalam bentuk pohon
lsblk

# Tampilkan detail partisi termasuk ukuran dan tipe
sudo fdisk -l

# Tampilkan tipe sistem berkas setiap partisi
lsblk -f
\end{lstlisting}
...
menunjukkan sistem berkas setiap partisi, misalnya \texttt{ext4} untuk partisi root
...
\begin{itemize}[noitemsep,topsep=2pt]
    ...
    \item \textbf{Arsitektur kernel}: kernel berjalan di ruang kernel dengan hak akses
    penuh; aplikasi di ruang pengguna berkomunikasi melalui system call; tiga arsitektur
    ...
    \item \textbf{Instalasi Ubuntu Server}: mesin virtual di VirtualBox menyediakan
    ...
    \item \textbf{Partisi dan sistem berkas}: disk dibagi menjadi partisi-partisi
    independen; GPT menggantikan MBR pada sistem modern; Linux menggunakan ext4 untuk
    partisi root, FAT32 untuk EFI, dan area swap sebagai memori virtual.
    ...
    \cmd{lsblk} memastikan sistem siap untuk praktikum selanjutnya.
\end{itemize}

\section{Tugas Praktikum}
\label{sec:tugas}

\textbf{Instruksi Umum}: Kerjakan seluruh tugas secara mandiri dan dokumentasikan setiap
langkah dengan tangkapan layar (screenshot). Simpan dokumentasi dalam format PDF dan
kumpulkan sesuai ketentuan yang diberikan dosen.

\begin{exercisebox}[1.1 Instalasi dan Verifikasi Sistem]
Lakukan instalasi Ubuntu Server 24.04 LTS di VirtualBox sesuai langkah pada
Praktikum 1.1 hingga 1.3. Setelah instalasi selesai:
\begin{enumerate}
...
\begin{exercisebox}[1.2 Konfigurasi Awal dan Verifikasi]
...
\begin{exercisebox}[1.3 Analisis Konsep Sistem Operasi]
...
\begin{enumerate}
    ...
    \item Jelaskan perbedaan antara ruang kernel dan ruang pengguna. Mengapa aplikasi
    ...
    terinstalasi. Catat nama partisi, tipe sistem berkas, dan ukuran setiap partisi.
