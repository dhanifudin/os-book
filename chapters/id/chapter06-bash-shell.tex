...
\chapterhero{cyan}{Bangun kebiasaan kerja terminal yang efisien lewat konfigurasi shell, PATH, alias, fungsi, dan teknik interaksi Bash yang lebih produktif.}{\faTerminal\quad shell}{\faRoute\quad PATH}{\faTags\quad alias}

Terminal yang kamu buka tidak langsung terhubung ke kernel Linux. Di antaranya ada
satu lapisan perangkat lunak yang membaca setiap baris yang kamu ketik, menafsirkannya,
lalu meminta kernel melakukan sesuatu. Lapisan itu disebut \term{shell}. Tanpa shell,
kamu harus berbicara langsung dengan kernel dalam format yang jauh lebih rendah dan
tidak ramah manusia.
...
penulisan skrip karena tersedia hampir di semua sistem Linux~\cite{bash2024}.

Bash bukan sekadar cara mengetik perintah satu per satu. Ia punya variabel, bisa
menjalankan skrip, menyimpan riwayat perintah, dan menyelesaikan nama file secara
otomatis. Bab ini membahas cara mengonfigurasi dan memanfaatkan semua kemampuan itu
sehingga pekerjaan di terminal menjadi lebih cepat dan lebih aman.
...
dengan penanda visual ANSI yang berbeda untuk tiap bagian:
\texttt{\textbackslash[\textbackslash 033[34m\textbackslash]} untuk direktori aktif
dan \texttt{\textbackslash[\textbackslash 033[32m\textbackslash]} untuk
waktu. Gunakan perintah \cmd{echo} dari Bab~2 untuk menguji apakah
terminal mendukung escape sequence ANSI dengan \cmd{echo -e \textbackslash
033[32mTes\textbackslash033[0m}. Pastikan prompt kembali ke tampilan normal
...
%% ------------------------------------------------------------------------
\section{Alias dan Shell Function}
...
\term{Shell function} adalah langkah berikutnya. \term{Function} bisa menerima argumen,
menjalankan beberapa perintah, menggunakan variabel lokal, dan mengembalikan
nilai. Gunakan \term{function} saat logika yang kamu butuhkan lebih dari sekadar
singkatan perintah. Dua cara mendefinisikan function yang keduanya valid:

\begin{lstlisting}[language=bash, caption={Dua cara mendefinisikan shell function}]
...
Di dalam function, \texttt{\$1}, \texttt{\$2}, dan seterusnya adalah
argumen yang diberikan saat function dipanggil. Gunakan \cmd{local} untuk
mendeklarasikan variabel yang hanya hidup di dalam function. Tanpa \cmd{local},
variabel di dalam function akan mengubah variabel global dengan nama yang sama.

Perintah \cmd{type} sangat berguna untuk memverifikasi apakah sebuah nama
adalah alias, function, perintah builtin Bash, atau program eksternal. Cukup
ketik \texttt{type nama} untuk melihat jenisnya.

\subsection*{Praktek 6.3: Buat Alias dan Fungsi Produktivitas}

Tujuan praktek ini adalah membuat alias dan function yang berguna untuk
...
disk lebih dari 80\%, tampilkan teks ``PERINGATAN'' dengan penekanan visual
