...
\chapterhero{orange}{Jelajahi struktur filesystem Linux, pahami jenis file dan tautan, lalu kuasai pencarian serta pengarsipan untuk pekerjaan harian.}{\faFolderOpen\quad FHS}{\faLink\quad symbolic link}{\faArchive\quad archive}

Sistem yang baru kamu instal di Bab~1 sudah memiliki ribuan berkas yang tersebar di
ratusan direktori. Ini bukan kekacauan, melainkan pola yang sudah distandarisasi
komunitas Linux. Administrator yang terbiasa dengan Ubuntu bisa langsung menemukan
file konfigurasi di mesin Debian yang belum pernah disentuhnya, karena keduanya
mengikuti aturan tata letak yang sama.

\term{Filesystem} Linux bukan tumpukan berkas yang diletakkan sembarangan. Setiap direktori punya
...
% ------------------------------------------------------------------------
\section{Standar Hierarki Filesystem}
\label{sec:fhs}
...
perangkat penyimpanan, partisi, bahkan \term{filesystem} jaringan, dipasang (\term{mount}) ke
dalam satu pohon yang sama. Standar yang mengatur tata letak ini disebut
\textit{Filesystem Hierarchy Standard} (FHS)~\cite{fhs2015}.
...
\begin{itemize}
    ...
    \item \dir{/usr/} : program dan \term{shared library} untuk pengguna. \dir{/usr/bin/} berisi sebagian besar aplikasi yang kamu instal.
    ...
    \item \dir{/sys/} : \term{filesystem} virtual lain dari kernel, berisi informasi perangkat keras dan \term{driver}.
...
\begin{itemize}
    \item \texttt{-} : berkas reguler. Dokumen teks, gambar, \term{binary} program, semua masuk kategori ini.
...
\term{binary}, atau sesuatu yang lain.
...
lengkap sebuah program yang ada di \var{PATH}, dan \cmd{whereis} menampilkan lokasi \term{binary},
