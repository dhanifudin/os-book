...
\begin{notebox}
Seluruh praktikum pada bab ini dijalankan pada direktori \dir{~/lab-os/week14-backup/}.
\begin{lstlisting}[language=bash]
mkdir -p ~/lab-os/week14-backup
cd ~/lab-os/week14-backup
\end{lstlisting}
\end{notebox}
...
\subsection*{Praktikum 14.1: Rencanakan Strategi Backup}

Tujuan praktikum ini adalah menyusun rencana backup tertulis menggunakan heredoc
dari Bab~3, lalu membuat data simulasi yang akan digunakan di praktikum-praktikum
berikutnya.
...
Penyimpanan    : lokal ~/lab-os/week14-backup/arsip/
...
rsync -av ~/lab-os/week14-backup/data-sumber/ ~/lab-os/week14-backup/arsip-rsync/
rsync -avn ~/lab-os/week14-backup/data-sumber/ ~/lab-os/week14-backup/arsip-rsync/
rsync -av --delete ~/lab-os/week14-backup/data-sumber/ ~/lab-os/week14-backup/arsip-rsync/
...
BACKUP_BASE=~/lab-os/week14-backup/snapshots
TODAY=$(date +%F)
...
rsync -av --delete \
    ...
    ~/lab-os/week14-backup/data-sumber/ \
    "$BACKUP_BASE/$TODAY/"
\end{lstlisting}
...
\subsection*{Praktikum 14.2: Sinkronisasi Direktori dengan rsync}

Tujuan praktikum ini adalah memahami perilaku \cmd{rsync} dengan berbagai opsi,
...
\end{lstlisting}
    ...
    \item Bersihkan snapshot setelah praktikum:
...
\begin{challengebox}
Buat skrip Bash (mengacu ke Bab~9) bernama \file{rsync-backup.sh} di direktori
...
\subsection*{Praktikum 14.3: Buat Arsip Backup dan Verifikasi Integritasnya}

Tujuan praktikum ini adalah membuat arsip backup penuh dan incremental,
...
BACKUP_DIR="$HOME/lab-os/week14-backup/arsip-otomatis"
SOURCE="$HOME/lab-os/week14-backup/data-sumber"
DATE=$(date +%Y-%m-%d-%H%M)
LOG="$HOME/lab-os/week14-backup/backup.log"
...
\subsection*{Praktikum 14.4: Jadwalkan Skrip Backup Otomatis}

Tujuan praktikum ini adalah membuat skrip backup, mengujinya secara manual,
...
#!/bin/bash
BACKUP_BASE="$HOME/lab-os/week14-backup/arsip-cron"
SOURCE="$HOME/lab-os/week14-backup/data-sumber"
DATE=$(date +%Y-%m-%d-%H%M)
LOG="$HOME/lab-os/week14-backup/cron-backup.log"
...
echo "*/2 * * * * $HOME/lab-os/week14-backup/backup-otomatis.sh" \
    >> /tmp/cron-sementara
...
\begin{challengebox}
Jadwalkan skrip \file{rsync-backup.sh} dari challengebox Praktikum 14.2
...
\subsection*{Praktikum 14.5: Simulasi Pemulihan dari Backup}

Tujuan praktikum ini adalah mensimulasikan skenario kehilangan data nyata,
...
\begin{lstlisting}[language=bash, caption={Membersihkan direktori praktikum}]
...
\begin{exercisebox}[14.1 Implementasi Sistem Backup Lengkap]
...
\begin{exercisebox}[14.2 Analisis Kompresi dan Performa Backup]
...
\begin{exercisebox}[14.3 Disaster Recovery Drill]
