...
\chapterhero{teal}{Kenali cara Linux mendeteksi perangkat keras, memuat driver, dan menyediakan perintah terminal dasar untuk mulai bekerja dengan percaya diri.}{\faMicrochip\quad hardware}{\faCogs\quad driver}{\faTerminal\quad terminal}

Linux tidak menerima keberadaan perangkat keras begitu saja. Saat komputer menyala,
kernel memeriksa setiap bus yang tersedia, mengenali perangkat yang terpasang, dan
memuat \term{driver} yang sesuai. Prosesor, RAM, kartu jaringan, disk, semua harus
dikenali sebelum sistem bisa dipakai. Di Windows, ringkasan ini tersedia di Device
Manager. Di Linux, seluruh jejaknya bisa kamu baca langsung dari terminal, dengan
tingkat detail yang jauh lebih dalam.
...
Memahami perangkat keras dan perintah dasar terminal adalah fondasi dari semua praktek Linux berikutnya. Tanpa keterampilan ini, kamu tidak akan bisa membaca informasi sistem, berpindah direktori dengan percaya diri, mengelola berkas sederhana, atau menafsirkan dokumentasi perintah yang kamu temui sepanjang buku ini.
...
\begin{notebox}
Seluruh praktek pada bab ini dijalankan pada direktori \dir{~/lab-os/chapter02-hardware/}.
...
Pesan kernel yang dihasilkan saat boot tersimpan dalam buffer cincin yang bisa dibaca dengan \cmd{dmesg}. Di sana kamu bisa melihat catatan deteksi perangkat: kapan disk ditemukan, \term{driver} apa yang dimuat, dan apakah ada perangkat yang gagal diinisialisasi. Tambahkan flag \texttt{-T} untuk menampilkan waktu dalam format yang bisa dibaca manusia.
...
\subsection*{Praktek 2.1: Identifikasi Komponen Perangkat Keras VM}

Tujuan praktek ini adalah membaca spesifikasi prosesor, memori, dan perangkat PCI dari \term{virtual machine}.
...
lspci -nnk
\end{lstlisting}
    \textit{Amati:} Setiap entri menampilkan ID vendor dan perangkat dalam tanda kurung, diikuti baris \texttt{Kernel driver in use} yang menunjukkan nama modul \term{driver} aktif.

    \item Cari kartu jaringan dan \term{driver} yang menanganinya:
...
Saat kernel berhasil memuat modul, hasilnya biasanya terlihat di pesan \cmd{dmesg}, di daftar perangkat PCI/USB, atau pada kemampuan baru yang tersedia di sistem. Karena itu, tiga alat yang paling sering dipakai bersama adalah \cmd{lsmod} untuk melihat apa yang aktif, \cmd{modinfo} untuk memahami modul tertentu, dan \cmd{dmesg} untuk menelusuri kapan \term{driver} tersebut dipakai atau menimbulkan error.

\begin{warningbox}
Jangan melepas modul yang sedang digunakan oleh sistem aktif. Melepas \term{driver} disk atau modul jaringan dapat menyebabkan sistem tidak responsif atau koneksi SSH terputus.
\end{warningbox}

\subsection*{Praktek 2.2: Eksplorasi Modul Aktif dan Driver Perangkat}

Tujuan praktek ini adalah melihat modul kernel yang sedang berjalan dan menghubungkannya dengan perangkat yang terdeteksi sistem.
...
\begin{challengebox}
Pilih satu perangkat dari output \cmd{lspci -nnk}. Catat nama perangkat dan \term{driver} aktifnya. Lalu gunakan \cmd{modinfo} untuk mencari deskripsi modul tersebut, kemudian verifikasi dengan \cmd{dmesg} apakah ada pesan boot yang menyebut perangkat atau \term{driver} yang sama.
\end{challengebox}
...
Terminal Linux adalah antarmuka utama administrasi server. Tidak seperti antarmuka grafis yang menyembunyikan detail, terminal memberi kontrol penuh: kamu tahu persis apa yang terjadi dan mengapa. Penguasaan perintah dasar bukan pilihan, melainkan keharusan~\cite{shotts2019linux}.
...
\subsection*{Praktek 2.3: Buat Struktur Direktori Latihan}

Tujuan praktek ini adalah membangun struktur direktori kerja dan melatih manipulasi berkas dasar.
...
\subsection*{Praktek 2.4: Filter dan Ekstrak Teks dari Output Sistem}

Tujuan praktek ini adalah menggunakan \cmd{grep}, \cmd{sed}, dan \cmd{awk} untuk mengolah output perintah sistem nyata.
...
\subsection*{Praktek 2.5: Telusuri Dokumentasi Perintah}

Tujuan praktek ini adalah membiasakan diri mencari dokumentasi dan memahami perbedaan sumber bantuan yang tersedia di Linux.
...
\subsubsection*{Latihan 2.1 Profil Perangkat Keras VM}
Jalankan \cmd{lspci -nnk} dan pilih satu perangkat PCI. Catat: (a) nama perangkat, (b) ID vendor dan perangkat dalam format heksadesimal, (c) nama \term{driver} kernel aktif. Kemudian jalankan \cmd{modinfo} untuk \term{driver} tersebut dan tuliskan deskripsi singkat serta nama penulis modulnya.


\subsubsection*{Latihan 2.2 Dokumentasi dan Modul Kernel}
...
\subsubsection*{Latihan 2.3 Analisis Log dengan grep/sed/awk}
