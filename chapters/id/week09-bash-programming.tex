...
Di Bab~7 kamu belajar cara bekerja lebih cepat di terminal: alias,
...
Bab ini membangun langsung di atas fondasi Bash dari Bab~7.
...
Semua teknik ini menjadi fondasi untuk Bab~10 di mana kamu akan membuat
...
\begin{notebox}
Seluruh praktikum pada bab ini dijalankan pada direktori
\dir{~/lab-os/week09-scripting/}.
Buat direktori ini sebelum memulai:
\begin{lstlisting}[language=bash]
mkdir -p ~/lab-os/week09-scripting/{skrip,logs,lib}
cd ~/lab-os/week09-scripting
\end{lstlisting}
\end{notebox}

%% ------------------------------------------------------------------------
\section{Dasar Scripting Bash}
\label{sec:dasar-scripting-bash}
...
\subsection*{Praktikum 9.1: Buat dan Jalankan Skrip Pertama}

Tujuan praktikum ini adalah membuat skrip Bash lengkap dengan shebang,
memberikan izin eksekusi, dan memahami exit code.

\begin{enumerate}
    \item Buat skrip pertama yang menampilkan informasi sistem dasar:
\begin{lstlisting}[language=bash, caption={Membuat skrip salam-sistem.sh}]
cat <<'EOF' > ~/lab-os/week09-scripting/skrip/salam-sistem.sh
#!/bin/bash
# salam-sistem.sh: Menampilkan salam dan info sistem dasar
# Penggunaan: ./salam-sistem.sh [nama-pengguna]
...
chmod +x ~/lab-os/week09-scripting/skrip/salam-sistem.sh

cd ~/lab-os/week09-scripting/skrip
./salam-sistem.sh
./salam-sistem.sh "Budi"
\end{lstlisting}
...
cat <<'EOF' > ~/lab-os/week09-scripting/skrip/cek-file.sh
#!/bin/bash
...
chmod +x ~/lab-os/week09-scripting/skrip/cek-file.sh
...
\subsection*{Praktikum 9.2: Gunakan Parameter Posisional}

Tujuan praktikum ini adalah membuat skrip yang memproses argumen dari
...
cat <<'EOF' > ~/lab-os/week09-scripting/skrip/cek-parameter.sh
#!/bin/bash
...
chmod +x ~/lab-os/week09-scripting/skrip/cek-parameter.sh
...
cat <<'EOF' > ~/lab-os/week09-scripting/skrip/statistik-angka.sh
#!/bin/bash
...
chmod +x ~/lab-os/week09-scripting/skrip/statistik-angka.sh
...
\subsection*{Praktikum 9.3: Tulis Skrip dengan Logika Percabangan dan Perulangan}

Tujuan praktikum ini adalah menggabungkan \cmd{if}, \cmd{for}, \cmd{while},
...
cat <<'EOF' > ~/lab-os/week09-scripting/skrip/cek-disk.sh
#!/bin/bash
...
chmod +x ~/lab-os/week09-scripting/skrip/cek-disk.sh
./cek-disk.sh
./cek-disk.sh 50
\end{longlisting}
...
cat <<'EOF' > ~/lab-os/week09-scripting/skrip/menu-monitor.sh
#!/bin/bash
...
chmod +x ~/lab-os/week09-scripting/skrip/menu-monitor.sh
\end{longlisting}
...
berisi file. Gunakan perintah \cmd{find} dari Bab~5 untuk memeriksa isi
...
\file{~/lab-os/week09-scripting/logs/laporan-etc.txt} menggunakan
...
\subsection*{Praktikum 9.4: Refaktor Skrip Menggunakan Fungsi}

Tujuan praktikum ini adalah memisahkan logika ke dalam fungsi dan
...
cat <<'EOF' > ~/lab-os/week09-scripting/lib/library.sh
#!/bin/bash
# library.sh: Kumpulan fungsi utilitas untuk skrip sistem
# Cara pakai: source ~/lab-os/week09-scripting/lib/library.sh
...
cat <<'EOF' > ~/lab-os/week09-scripting/skrip/laporan-disk.sh
#!/bin/bash
# laporan-disk.sh: Laporan penggunaan disk menggunakan library

LIB_PATH="$HOME/lab-os/week09-scripting/lib/library.sh"
...
OUTPUT_DIR="$HOME/lab-os/week09-scripting/logs"
...
chmod +x ~/lab-os/week09-scripting/skrip/laporan-disk.sh
./laporan-disk.sh
\end{longlisting}
...
source ~/lab-os/week09-scripting/lib/library.sh
...
\begin{challengebox}
Pisahkan semua fungsi dari skrip \cmd{cek-disk.sh} di Praktikum 9.3
...
\subsection*{Praktikum 9.5: Debug Skrip Bermasalah}

Tujuan praktikum ini adalah menggunakan teknik debugging untuk
...
cat <<'EOF' > ~/lab-os/week09-scripting/skrip/proses-log-bermasalah.sh
#!/bin/bash
...
chmod +x ~/lab-os/week09-scripting/skrip/proses-log-bermasalah.sh

# Tanpa argumen (bug pertama)
bash -x ~/lab-os/week09-scripting/skrip/proses-log-bermasalah.sh

# Dengan direktori yang ada
bash -x ~/lab-os/week09-scripting/skrip/proses-log-bermasalah.sh \
  ~/lab-os/week09-scripting/logs
\end{lstlisting}
...
cat <<'EOF' > ~/lab-os/week09-scripting/skrip/proses-log-aman.sh
#!/bin/bash
set -euo pipefail
...
chmod +x ~/lab-os/week09-scripting/skrip/proses-log-aman.sh

# Buat beberapa file log untuk pengujian
echo -e "INFO: skrip mulai\nWARN: disk 75%\nINFO: selesai" \
    > ~/lab-os/week09-scripting/logs/test-a.log
echo -e "ERROR: koneksi gagal\nINFO: retry" \
    > ~/lab-os/week09-scripting/logs/test-b.log

./proses-log-aman.sh ~/lab-os/week09-scripting/logs
\end{longlisting}
...
\begin{challengebox}
Ambil skrip \cmd{cek-disk.sh} dari Praktikum 9.3 dan tingkatkan dengan
...
yang akan kamu temukan di Bab~10.
\end{challengebox}
...
\textbf{Instruksi Umum}: kerjakan seluruh tugas di
\dir{~/lab-os/week09-scripting/}. Simpan skrip di \dir{skrip/}, library
...
\begin{exercisebox}[9.1 Skrip Absensi dengan Laporan]
...
\begin{exercisebox}[9.2 Library dan Skrip Backup]
...
\begin{exercisebox}[9.3 Skrip Monitor Sistem dengan Debug]
