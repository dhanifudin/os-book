...
\begin{notebox}
Seluruh praktikum pada bab ini dijalankan pada direktori \dir{~/lab-os/week06-processes/}.
Buat direktori ini sebelum memulai:
\begin{lstlisting}[language=bash]
mkdir -p ~/lab-os/week06-processes
cd ~/lab-os/week06-processes
\end{lstlisting}
\end{notebox}
...
\subsection*{Praktikum 6.1 Amati Hierarki Proses dengan ps dan pstree}

Tujuan praktikum ini adalah melihat hierarki proses secara langsung dan memahami hubungan
...
Berikut kondisi-kondisi proses yang akan sering kamu temui:
...
\subsection*{Praktikum 6.2 Buat dan Amati Proses di Background}

Tujuan praktikum ini adalah mengamati kondisi proses secara langsung dan memahami
...
\subsection*{Praktikum 6.3 Ubah Prioritas Proses yang Berjalan}

Tujuan praktikum ini adalah menggunakan \cmd{nice} dan \cmd{renice} untuk mengatur
...
Jawabannya adalah \textbf{sinyal}. Sinyal adalah notifikasi singkat yang dikirim ke sebuah
proses untuk memintanya melakukan sesuatu: berhenti, dijeda, memuat ulang konfigurasi,
atau tindakan lainnya.

Setiap sinyal punya nama dan nomor. Kamu bisa menggunakan keduanya secara bergantian.
Berikut sinyal-sinyal yang paling sering digunakan:
...
permintaan sopan kepada proses untuk berhenti. Proses bisa menangkap sinyal ini, menyimpan
data yang belum tersimpan, menutup koneksi yang terbuka, lalu berhenti dengan bersih.
SIGKILL adalah perintah paksa dari kernel: proses tidak bisa menolak atau menangkap sinyal
...
\subsection*{Praktikum 6.4 Kirim Sinyal ke Proses}

Tujuan praktikum ini adalah mengirim berbagai jenis sinyal ke proses dan mengamati
...
Beri izin eksekusi menggunakan \cmd{chmod u+x} (dari Bab~5), jalankan di background,
lalu kirim SIGTERM ke PID-nya. Amati pesan yang muncul. Ini adalah gambaran awal tentang
penanganan sinyal dalam skrip yang akan kamu pelajari lebih lanjut di Bab~9.
\end{challengebox}
...
Berikut perintah-perintah manajemen job yang perlu kamu kuasai:
...
\subsection*{Praktikum 6.5 Kelola Beberapa Job Sekaligus}

Tujuan praktikum ini adalah mengelola job foreground dan background, serta menjalankan
...
nohup bash -c 'for i in $(seq 1 60); do echo "$(date): iterasi $i"; sleep 2; done' \
    > ~/lab-os/week06-processes/proses-panjang.log 2>&1 &
\end{lstlisting}
...
\textbf{Instruksi Umum}: Kerjakan seluruh tugas secara mandiri. Dokumentasikan setiap
langkah dengan tangkapan layar dan kumpulkan dalam format PDF.

\begin{exercisebox}[6.1 Eksplorasi Hierarki Proses]
...
\begin{exercisebox}[6.2 Simulasi Manajemen Job]
...
\begin{exercisebox}[6.3 Prioritas dan Sinyal]
